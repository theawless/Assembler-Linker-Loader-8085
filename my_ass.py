    ...
    if _pass == 1:
        ...
        for fileName in {"mys.x"}:
            ...
            for line in lines:
                ...
                if var.match(line):
                    ...
                    symTable[fileName][ext.match(line).group(1).lstrip().rstrip()] = '$' + str(
        ...
        for fileName in {"mys.x"}:
            ...
            for line in lines:
                line = line.lstrip().rstrip()
                if var.match(line):
                    address = symTable[fileName][var.match(line).group(1).lstrip().rstrip()].split('#')[1]
                    newCode.append('JMP #' + str(int(address) + 1))
                    newCode.append('DB ' + var.match(line).group(2).lstrip().rstrip())
                elif glo.match(line):
                    address = symTable[fileName][glo.match(line).group(1).lstrip().rstrip()].split('#')[1]
                    globTable[fileName][glo.match(line).group(1).lstrip().rstrip()] = '#' + str(int(address) + 3)
                    newCode.append('JMP #' + str(int(address) + 1))
